...
export async function processGeneration(ctx, session) {
  try {
    ...
    // За замовчуванням використовуємо KlingAI для відео
    // Veo залишається в коді для майбутнього використання
    const isVideo = contentType === 'video' || contentType === 'kling';
    const isKlingAI = contentType === 'kling' || contentType === 'video'; // За замовчуванням KlingAI
    ...
    if (isVideo) {
      ...
      // За замовчуванням використовуємо KlingAI для відео
      await ctx.reply(`Готово! Ось твоє відео для Reels 🎬✨ (згенеровано через KlingAI 1.6)`);
    ...
    // Формуємо повідомлення про статус генерацій
    let statusMessage = '';
    ...
    if (isFreeGeneration) {
      ...
      if (remainingFree > 0) {
        statusMessage = `🎁 Залишилось безкоштовних генерацій: ${remainingFree}`;
      } else {
        statusMessage = `💳 Безкоштовні генерації вичерпано.\n\nНаступні креативи коштуватимуть ${config.payment.amount} грн за 1 генерацію (2 варіанти зображень).`;
      }
    } else {
      ...
      if (updatedAvailablePaid > 0) {
        statusMessage = `💳 Використано 1 оплачену генерацію.\n\nЗалишилось оплачених генерацій: ${updatedAvailablePaid}`;
      } else {
        // Не показуємо оплату тут - вона буде показана при натисканні кнопок "Згенерувати ще раз" або "Змінити стиль"
        statusMessage = `💳 Використано останню оплачену генерацію.\n\nДля наступних креативів потрібна оплата ${config.payment.amount} грн за 1 генерацію (2 варіанти зображень).`;
      }
    }

    // Показуємо кнопки дій разом зі статусом генерацій
